import { CallExpr, Expr, Program, Type } from "./ast.js";
...
interface FunctionSlot {
  ...
  body?: Expr;
  isBuiltin?: boolean;
  fromBlockCompose?: boolean;
  composedFrom?: { a: string; b: string; key: string; lazy: boolean };
  locals: Map<string, TypeSlot>;
}
...
const booleanType: Type = { kind: "boolean" };

const builtins: FunctionSlot[] = [
...
function slot(type?: Type): TypeSlot {
  return { type: type ?? null };
}

function paramSlot(name: string, type?: Type): ParamSlot {
  return { name, slot: slot(type) };
}

function builtin(name: string, paramNames: string[], params: Type[], result: Type): FunctionSlot {
  return {
    name,
...
export function typecheck(program: Program): TypedProgram {
  const functions = new Map<string, FunctionSlot>();
  for (const b of builtins) functions.set(b.name, b);

  const exports: string[] = [];
  for (const stmt of program.statements) {
    if (stmt.kind === "fn") {
      if (functions.has(stmt.name)) throw new Error(`Duplicate function ${stmt.name}`);
  ...
  const typedFunctions = new Map<string, FunctionInfo>();
  for (const [name, fn] of functions) {
    const params = fn.params.map((p) => {
      if (!p.slot.type) throw new Error(`Could not infer type for parameter ${p.name} of ${name}`);
      return { name: p.name, type: p.slot.type };
    });
    if (!fn.returnType.type) throw new Error(`Could not infer return type for ${name}`);
    const locals = new Map<string, Type>();
    for (const [localName, localSlot] of fn.locals) {
      if (!localSlot.type) throw new Error(`Could not infer type for local ${localName} of ${name}`);
...
function buildCompose(name: string, a: FunctionSlot, b: FunctionSlot, key: string, lazy: boolean): FunctionSlot {
  const index = a.params.findIndex((param) => param.name === key);
  if (index < 0) throw new Error(`Compose key '${key}' not found on ${a.name}`);

  const combined = [
    ...a.params.filter((p) => p.name !== key).map((p) => ({ param: p, origin: a.name })),
    ...b.params.map((p) => ({ param: p, origin: b.name }))
  ];
  const baseName = (name: string) => (name.includes(".") ? name.split(".").pop()! : name);
  const baseCounts = new Map<string, number>();
  for (const item of combined) {
    const base = baseName(item.param.name);
    baseCounts.set(base, (baseCounts.get(base) ?? 0) + 1);
  }
  const newParams: ParamSlot[] = [];
  ...
  for (const item of combined) {
    ...
    if (used.has(finalName)) throw new Error(`Duplicate param ${finalName}`);
    used.add(finalName);
    newParams.push({ name: finalName, slot: item.param.slot });
  }

  const slotNameMap = new Map<TypeSlot, string>();
  for (const param of newParams) {
    slotNameMap.set(param.slot, param.name);
  }

  const bArgs: Expr[] = b.params.map((param) => ({
    kind: "var",
    name: slotNameMap.get(param.slot) ?? param.name
  }));
  const bCall: Expr = { kind: "call", name: b.name, args: bArgs };

  const aArgs: Expr[] = a.params.map((param) => {
    if (param.name === key) return bCall;
    return { kind: "var", name: slotNameMap.get(param.slot) ?? param.name };
  });

  const body: Expr = { kind: "call", name: a.name, args: aArgs };
...
function resolveKey(fn: FunctionSlot, key: string): string {
  if (key.includes(".")) {
    const match = fn.params.find((p) => p.name === key);
    if (!match) throw new Error(`Compose key '${key}' not found on ${fn.name}`);
    return match.name;
  }
  const matches = fn.params.filter((p) => p.name === key || p.name.endsWith(`.${key}`));
  if (matches.length === 0) throw new Error(`Compose key '${key}' not found on ${fn.name}`);
  if (matches.length > 1) {
...
function inferAll(functions: Map<string, FunctionSlot>) {
  let changed = true;
  while (changed) {
    changed = false;
    for (const [name, fn] of functions) {
      if (fn.isBuiltin || !fn.body) continue;
      changed = inferFunction(fn, functions) || changed;
    }
  }
}

function inferFunction(fn: FunctionSlot, functions: Map<string, FunctionSlot>): boolean {
  ...
  const env = new Map<string, TypeSlot>(fn.params.map((param) => [param.name, param.slot]));
  const state = { changed: false };
  const bodyType = inferExpr(fn.body!, env, functions, fn.returnType.type, state, fn);
  if (bodyType) unifySlot(fn.returnType, bodyType, state, `return type of ${fn.name}`);
  return state.changed;
}

function inferExpr(
  expr: Expr,
  env: Map<string, TypeSlot>,
  functions: Map<string, FunctionSlot>,
  expected: Type | null,
  ...
  if (expr.kind === "var") {
    ...
    if (!slot) throw new Error(`Unknown variable ${expr.name}`);
    if (expected) unifySlot(slot, expected, state, `variable ${expr.name}`);
    return slot.type ?? expected;
  }
  if (expr.kind === "block") {
    const exprItems = expr.items.filter((item) => item.kind !== "assign") as Expr[];
    if (exprItems.length !== 1) {
      throw new Error("Blocks must contain exactly one expression");
    }
    for (const item of expr.items) {
      if (item.kind !== "assign") continue;
      if (isComposeCall(item.expr)) {
        ...
        if (!fa || !fb) throw new Error(`Unknown function in compose ${a} or ${b}`);
      ...
      const slot = { type: null } as TypeSlot;
      const valueType = inferExpr(item.expr, env, functions, null, state, owner);
      if (valueType) slot.type = valueType;
      env.set(item.name, slot);
      if (owner) owner.locals.set(item.name, slot);
    }
    if (isComposeCall(exprItems[0])) {
      if (!owner) throw new Error("compose must be used in a function body");
  ...
  if (expr.kind === "call") {
    ...
    if (expr.name === "ternary") {
      if (expr.args.length !== 3) throw new Error(`ternary expects 3 args`);
      const condType = inferExpr(expr.args[0], env, functions, booleanType, state, owner);
      if (condType && !typeEquals(condType, booleanType)) throw new Error(`ternary condition must be boolean`);
...
function applyComposeBody(fn: FunctionSlot, expr: Expr, functions: Map<string, FunctionSlot>): boolean {
...
function isComposeCall(expr: Expr): boolean {
  return (
    expr.kind === "call" &&
...
function parseComposeCall(expr: Expr): { a: string; b: string; key?: string; lazy: boolean; hasKey: boolean } {
  if (!isComposeCall(expr)) throw new Error("Invalid compose call");
  const call = expr as Extract<Expr, { kind: "call" }>;
...
function validateComposeTypes(functions: Map<string, FunctionSlot>) {
  for (const fn of functions.values()) {
    if (!fn.composedFrom) continue;
    ...
    if (fn.composedFrom!.lazy) {
      if (b.returnType.type.kind === "array") {
        if (!typeEquals(b.returnType.type.element, aParam.slot.type)) {
          throw new Error(
            `lazy_compose type mismatch: ${fn.composedFrom.a}.${fn.composedFrom.key} expects ${typeToString(
              aParam.slot.type
            )} but ${fn.composedFrom.b} returns ${typeToString(b.returnType.type)}`
          );
        }
      ...
      throw new Error(
        `compose type mismatch: ${fn.composedFrom.a}.${fn.composedFrom.key} expects ${typeToString(
          aParam.slot.type
        )} but ${fn.composedFrom.b} returns ${typeToString(b.returnType.type)}`
      );
    }
...
function unifySlot(slot: TypeSlot, type: Type, state: { changed: boolean }, label: string) {
  if (!slot.type) {
    slot.type = type;
    state.changed = true;
    return;
...
function mergeTypes(a: Type | null, b: Type | null, expected: Type | null): Type | null {
  if (a && b) {
    if (!typeEquals(a, b)) throw new Error(`Type mismatch: ${typeToString(a)} vs ${typeToString(b)}`);
    return a;
  }
  return a ?? b ?? expected ?? null;
}

function literalToType(value: number | boolean | string | null): Type {
...
function enforceAcyclic(functions: Map<string, FunctionSlot>) {
  const visiting = new Set<string>();
  const visited = new Set<string>();

  const visit = (name: string) => {
    if (visited.has(name)) return;
    if (visiting.has(name)) throw new Error(`Recursion not allowed: ${name}`);
    visiting.add(name);
    const fn = functions.get(name);
    if (fn?.body) {
      for (const dep of collectDeps(fn.body)) {
        if (functions.get(dep)?.isBuiltin) continue;
        visit(dep);
      }
    }
  ...
  for (const [name, fn] of functions) {
    if (fn.isBuiltin) continue;
    visit(name);
  }
}

function collectDeps(expr: Expr): Set<string> {
  const deps = new Set<string>();
  const walk = (e: Expr) => {
    if (e.kind === "block") {
      for (const item of e.items) {
        if (item.kind === "assign") {
          if (isComposeCall(item.expr)) continue;
          walk(item.expr);
          continue;
        }
        walk(item as Expr);
    ...
    if (e.kind === "call") {
      if (e.name === "compose" || e.name === "lazy_compose") return;
      deps.add(e.name);
...
function collectFreeVars(expr: Expr): string[] {
...
export function typeEquals(a: Type, b: Type): boolean {
  ...
  if (a.kind === "object" && b.kind === "object") {
    ...
    for (const key of aKeys) {
      if (!b.fields[key]) return false;
      if (!typeEquals(a.fields[key], b.fields[key])) return false;
    }
    return true;
  }
  return true;
