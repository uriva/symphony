import { Expr, Type } from "./ast.js";
import { FunctionInfo } from "./typecheck.js";
...
export function emitWat(functions: Map<string, FunctionInfo>, exportsList: string[]): string {
  const lowered: LoweredFunction[] = [];
  const returnTypes = new Map<string, Type>();

  for (const [name, fn] of functions) {
    if (fn.isBuiltin) continue;
    if (!fn.body) continue;
    ensureWasmType(fn.returnType);
    for (const param of fn.params) ensureWasmType(param.type);
...
function emitExpr(expr: Expr, fn: LoweredFunction, returnTypes: Map<string, Type>): string[] {
  if (expr.kind === "literal") {
    if (typeof expr.value === "number") return [`f64.const ${expr.value}`];
    if (typeof expr.value === "boolean") return [`i32.const ${expr.value ? 1 : 0}`];
...
function mapWatType(type: Type): string {
  if (type.kind === "number") return "f64";
  if (type.kind === "boolean") return "i32";
  throw new Error(`Type ${type.kind} not supported in Wasm backend`);
}

function ensureWasmType(type: Type) {
  if (type.kind === "number" || type.kind === "boolean") return;
  throw new Error(`Type ${type.kind} not supported in Wasm backend`);
}

function inferWatType(expr: Expr, fn: LoweredFunction, returnTypes: Map<string, Type>): string {
  if (expr.kind === "literal") {
    if (typeof expr.value === "number") return "f64";
    if (typeof expr.value === "boolean") return "i32";
    throw new Error(`Literal type not supported`);
  }
  if (expr.kind === "var") {
    const index = fn.paramNames.findIndex((name) => name === expr.name);
    if (index < 0) throw new Error(`Unknown variable ${expr.name}`);
  ...
  if (expr.kind === "call") {
    ...
    if (expr.name === "+" || expr.name === "-" || expr.name === "*" || expr.name === "/") return "f64";
    if (expr.name === "ternary") return inferWatType(expr.args[1], fn, returnTypes);
    const ret = returnTypes.get(expr.name);
    if (!ret) throw new Error(`Unknown function ${expr.name}`);
